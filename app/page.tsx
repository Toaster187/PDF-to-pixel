...
import { renderPageToPng } from '@/lib/png-renderer';
...
function riskFor(plan: OutputPlan): 'normal' | 'large' | 'extreme' {
  if (plan.rawBytes >= 2 * 1024 ** 3 || Math.max(plan.width, plan.height) > 65_535) return 'extreme';
  if (plan.rawBytes >= 512 * 1024 ** 2 || Math.max(plan.width, plan.height) > 32_768) return 'large';
  return 'normal';
}
    ...
    <aside className="self-start rounded-[28px] border border-border bg-card p-5 shadow-[0_24px_70px_rgb(27_33_45/7%)] lg:sticky lg:top-6">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold tracking-[-0.025em]">Qualitätsregeln</h2>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-secondary/55 p-4">
          <div className="flex items-center gap-3">
            ...
            <Switch
              ...
              disabled={disabled}
              onCheckedChange={(checked) => onChange({ ...settings, protectImages: checked })}
            />
          </div>
          <p className="mt-2 text-xs leading-5 text-muted-foreground">
            Jedes platzierte Rasterbild erhält mindestens so viele Ausgabepixel wie seine native
            Quelle.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-secondary/55 p-4">
          ...
          <Slider
            ...
            onValueChange={(value) =>
              onChange({
                ...settings,
                minimumTextPixels: Array.isArray(value) ? value[0] : Number(value),
              })
            }
          />
          <div className="mt-2 flex justify-between font-mono text-[9px] text-muted-foreground">
      ...
      <div className="mt-5 flex gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-blue-950">
        ...
        <p className="text-xs leading-5">
          Kein Upload, kein Konto, keine KI-Inferenz. PDF-Analyse, Rendering und Kompression laufen
          vollständig auf diesem Gerät.
        </p>
      </div>
    </aside>
  );
      ...
      <div className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 text-xl font-semibold tracking-[-0.035em]">
              {numberFormatter.format(plan.width)} × {numberFormatter.format(plan.height)} px
            </h3>
          </div>
          {risk !== 'normal' && (
            <span
        ...
        <dl className="mt-5 grid grid-cols-2 gap-2">
          <div className="rounded-xl bg-secondary/65 p-3">
            ...
            <dd className="mt-1 text-xs font-semibold">{driverLabel(plan.driver)}</dd>
          </div>
          <div className="rounded-xl bg-secondary/65 p-3">
            <dt className="font-mono text-[9px] uppercase tracking-[0.12em] text-muted-foreground">
              Raster
            </dt>
            <dd className="mt-1 text-xs font-semibold">{numberFormatter.format(Math.round(plan.dpi))} dpi</dd>
          </div>
          <div className="rounded-xl bg-secondary/65 p-3">
            <dt className="font-mono text-[9px] uppercase tracking-[0.12em] text-muted-foreground">
              Pixelmenge
            </dt>
            <dd className="mt-1 text-xs font-semibold">{decimalFormatter.format(plan.megapixels)} MP</dd>
          </div>
          <div className="rounded-xl bg-secondary/65 p-3">
            <dt className="font-mono text-[9px] uppercase tracking-[0.12em] text-muted-foreground">
              Rohdaten
            </dt>
            <dd className="mt-1 text-xs font-semibold">{formatBytes(plan.rawBytes)}</dd>
          </div>
        </dl>
...
export default function Home() {
  ...
  const loadFile = useCallback(
    async (file: File) => {
      ...
      if (previousDocument) await previousDocument.loadingTask.destroy().catch(() => undefined);
      ...
      try {
        const pdf = await openLocalPdf(
          ...
          (loaded, total) => setProgress(Math.min(0.18, (loaded / Math.max(total, 1)) * 0.18)),
        );
        ...
        const completed: PageScan[] = [];
        for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
          if (token !== loadTokenRef.current) return;
          const page = await pdf.getPage(pageNumber);
          const [scan, previewUrl] = await Promise.all([scanPage(page), createPagePreview(page)]);
          if (token !== loadTokenRef.current) {
            URL.revokeObjectURL(previewUrl);
            return;
          }
          previewUrlsRef.current.push(previewUrl);
          completed.push({ ...scan, previewUrl });
          setScans([...completed]);
          setProgress(0.18 + (pageNumber / pdf.numPages) * 0.82);
          setStatus(`${pageNumber} von ${pdf.numPages} Seiten analysiert`);
        }

        setPhase('ready');
        setProgress(1);
        setStatus(`${pdf.numPages} ${pdf.numPages === 1 ? 'Seite ist' : 'Seiten sind'} bereit`);
      } catch (caught) {
        if (token !== loadTokenRef.current) return;
        const message = caught instanceof Error ? caught.message : 'Die PDF konnte nicht gelesen werden.';
        setError(
  ...
  const reset = useCallback(async () => {
    ...
    documentRef.current = null;
    setDocumentProxy(null);
    if (currentDocument) await currentDocument.loadingTask.destroy().catch(() => undefined);
    setFileInfo(null);
    setScans([]);
    setError(null);
    setProgress(0);
  ...
  const confirmLargeExport = useCallback((plans: OutputPlan[], label: string) => {
    const totalRaw = plans.reduce((sum, plan) => sum + plan.rawBytes, 0);
    const longestEdge = Math.max(...plans.map((plan) => Math.max(plan.width, plan.height)));
    if (totalRaw < 1024 ** 3 && longestEdge <= 32_768) return true;
    return window.confirm(
      `${label} benötigt bis zu ${formatBytes(totalRaw)} ungefilterte RGB-Daten und kann lange dauern. ` +
        'Das fertige PNG bzw. ZIP ist dank verlustfreier Kompression meist kleiner. Trotzdem starten?',
    );
  }, []);
  ...
  const exportSingle = useCallback(
    async (pageNumber: number) => {
      ...
      const item = plannedPages.find(({ scan }) => scan.pageNumber === pageNumber);
      ...
      setExportState({
        ...
        pageNumber,
        progress: 0,
        message: `Seite ${pageNumber} wird vorbereitet …`,
      });
      ...
      try {
        ...
        const blob = await renderPageToPng(page, {
          ...
          onProgress: ({ ratio, message }) =>
            setExportState({ scope: 'single', pageNumber, progress: ratio, message }),
        });
        const baseName = cleanBaseName(fileInfo?.name ?? 'pdf-export');
        downloadBlob(blob, `${baseName}-seite-${String(pageNumber).padStart(2, '0')}.png`);
      } catch (caught) {
  ...
  const exportAll = useCallback(async () => {
    const pdf = documentRef.current;
    if (!pdf || !plannedPages.length || exportState || phase !== 'ready') return;
    if (!confirmLargeExport(plannedPages.map(({ plan }) => plan), 'Alle Seiten zusammen')) return;

    const controller = new AbortController();
    exportAbortRef.current = controller;
    setError(null);
    setExportState({ scope: 'all', progress: 0, message: 'ZIP wird vorbereitet …' });

    try {
      const zipParts: BlobPart[] = [];
      let finishZip: ((blob: Blob) => void) | undefined;
      ...
      const zip = new Zip((zipError, chunk, final) => {
        ...
        zipParts.push(chunk.slice());
        if (final) finishZip?.(new Blob(zipParts, { type: 'application/zip' }));
      });
      const baseName = cleanBaseName(fileInfo?.name ?? 'pdf-export');

      for (let index = 0; index < plannedPages.length; index += 1) {
        if (controller.signal.aborted) throw new DOMException('Export abgebrochen', 'AbortError');
      ...
      zip.end();
      const archive = await zipFinished;
      downloadBlob(archive, `${baseName}-alle-seiten.zip`);
    } catch (caught) {
      if (!(caught instanceof DOMException && caught.name === 'AbortError')) {
        setError(
          caught instanceof Error
  ...
  const totalPixels = plannedPages.reduce((sum, { plan }) => sum + plan.megapixels, 0);

  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border/80 bg-background/95">
        <div className="mx-auto flex h-16 max-w-[1480px] items-center justify-between px-5 lg:px-8">
          <button className="flex items-center gap-3 text-left" onClick={() => void reset()}>
            ...
            <span>
              <span className="block font-semibold leading-none tracking-[-0.02em]">PDF / PIXEL</span>
      ...
      {!fileInfo ? (
        <section className="mx-auto grid max-w-[1480px] gap-6 px-5 py-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-8">
          <div className="min-w-0">
            <div className="mb-7 max-w-3xl">
              ...
              <p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">
                Die Seite prüft native Bildauflösungen und kleinen Text, berechnet daraus die
                nötige Ausgabegröße und rendert jede PDF-Seite direkt in deinem Browser.
              </p>
            </div>
            <DropSurface
          ...
          <SettingsPanel settings={settings} onChange={setSettings} disabled={false} />
        </section>
      ) : (
          ...
          <div className="mb-6 rounded-[24px] border border-border bg-card p-4 shadow-[0_18px_55px_rgb(27_33_45/6%)] sm:p-5">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center">
              <div className="flex min-w-0 items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="truncate font-semibold tracking-[-0.02em]">{fileInfo.name}</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {formatBytes(fileInfo.size)} · {fileInfo.pages || '…'} Seiten · nur lokal geöffnet
                  </p>
                </div>
              </div>
            ...
            <div className="fixed inset-x-4 bottom-4 z-50 mx-auto max-w-xl rounded-[22px] border border-blue-200 bg-white/96 p-4 shadow-[0_25px_90px_rgb(27_33_45/25%)] backdrop-blur-xl sm:bottom-6">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold">{exportState.message}</p>
                  <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-secondary">
                    <div
                      className="h-full rounded-full bg-primary transition-[width]"
                      style={{ width: `${Math.max(1, exportState.progress * 100)}%` }}
                    />
                  </div>
                </div>
                <Button
            ...
            <div className="min-w-0">
              ...
              {isWorking && scans.length === 0 ? (
                <div className="grid min-h-[420px] place-items-center rounded-[28px] border border-border bg-card p-8 text-center shadow-[0_18px_55px_rgb(27_33_45/6%)]">
                  <div>
                    ...
                    <h2 className="mt-5 text-xl font-semibold">Details werden vermessen</h2>
                  ...
                  {phase === 'analyzing' && scans.length < fileInfo.pages && (
                    <div className="grid min-h-[420px] place-items-center rounded-[26px] border border-dashed border-border bg-card/60 p-8 text-center">
                      <div>
                        ...
                        <p className="mt-3 text-sm font-medium">Nächste Seite wird analysiert …</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="mt-6 rounded-[24px] border border-border bg-card p-5 sm:p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h2 className="font-semibold tracking-[-0.02em]">Was „verlustfrei“ hier genau bedeutet</h2>
                    <p className="mt-2 text-sm leading-6 text-muted-foreground">
                      PNG komprimiert ohne JPEG-Verluste. Eingebettete Bilder werden nicht unter ihre
                      native Detaildichte verkleinert. Text und Vektoren sind mathematisch unbegrenzt
                      skalierbar – deshalb definieren die dpi-Basis und die Mindesthöhe eine endliche,
                      sehr scharfe Rasterausgabe. Ein einzelnes PNG kann keine unterschiedlichen
                      Pixelraster pro Element besitzen: Ein 720p-Bild gewinnt im großen Seitenraster
                      keine erfundenen Details, seine einfacheren Bilddaten werden aber verlustfrei
                      mitkomprimiert.
                    </p>
                  </div>
                </div>
